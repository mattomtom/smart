...
namespace Smart.iOS
{
    public class ViewControllerAnimationAllSmooth : UIViewController
    {
        ...
        public override void ViewDidLoad()
        {
            ...
            View.AddGestureRecognizer(new UIPanGestureRecognizer(pan =>
            {
                ...
                _verticleLine.Path = GetLinePathWithAmount((float)p.X, (float)p.Y, _lineMargin);

                Debug.WriteLine("State :{0}", pan.State);
                if (pan.State == UIGestureRecognizerState.Ended)
                {
                    AnimateLineReturnFrom((float)p.X, (float)p.Y, _lineMargin);
                }

            }));

            void AnimateLineReturnFrom(float positionX, float positionY, float maring)
            {
                // TODO: animacja :)                   
                _morph.TimingFunction = CAMediaTimingFunction.FromName(CAMediaTimingFunction.EaseIn);
                List<CGPath> list = new List<CGPath>();
                list.Add(GetLinePathWithAmount(positionX, positionY, maring));
                list.Add(GetLinePathWithAmount(positionX, positionY * 0.9f, maring));
                list.Add(GetLinePathWithAmount(positionX, positionY * 0.6f, maring));
                list.Add(GetLinePathWithAmount(positionX, positionY * 0.4f, maring));
                list.Add(GetLinePathWithAmount(positionX, positionY * 0.25f, maring));
                list.Add(GetLinePathWithAmount(0, 0, maring));
            ...
            _morph.AnimationStopped += delegate {
                _verticleLine.Path = GetLinePathWithAmount(0, 0, _lineMargin);
            ...
            CAShapeLayer CreateLine()
            {
                ...
                _verticleLine.Path = GetLinePathWithAmount(0, 0, 0);
                _verticleLine.FillColor = UIColor.Red.CGColor;

                return _verticleLine;
            }

            CGPath GetLinePathWithAmount(float amountX, float amountY, float margin)
            {
                UIBezierPath verticleLine = new UIBezierPath();
                CGPoint topPoint = new CGPoint(0 - margin, 0);
                CGPoint midControlPoint = new CGPoint(amountX, amountY);
